...
extern "C" {
    s21::model* create_model() {
        return new s21::model();
    }

    void destroy_model(s21::model* model) {
        delete model;
    }

    void set_expression(s21::model* model, const char* expression) {
        model->setExpression(std::string(expression));
    }

    void set_var_x(s21::model* model, double x) {
        model->setVarX(x);
    }

    void set_step(s21::model* model, double step) {
        model->setStep(step);
    }

    // Добавьте другие методы аналогично...
    std::pair<std::vector<double>, std::vector<double>> plot(s21::model* model) {
        return model->plot();
    }

    const char* calculate(s21::model* model) {
        std::string result = model->calculate();
